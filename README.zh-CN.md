...
![CSM TUI 预览](assets/csm-tui-preview.zh-CN.svg)
...
对大多数日常会话切换、provider/model 切换、上下文修复场景，优先使用
`smart`。

`smart` 是 CSM 里最重要的高层工作流。它会提供 provider/model 选择向导，
先展示执行计划预览，再自动决定应该走“同线程修复”还是“原生迁移”路径。

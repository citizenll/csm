...
#[test]
fn cli_parses_first_token_preview_command() {
    let cli = crate::Cli::try_parse_from([
        "codex-session-manager",
        "first-token-preview",
        "thread-123",
    ...
    let Some(crate::cli::Command::FirstTokenPreview(args)) = cli.command else {
        panic!("expected first-token-preview command");
    };

    assert_eq!(args.target.target, "thread-123");
    assert_eq!(args.input.as_deref(), Some("continue with the migration"));
    assert!(args.json);
}
